...
seqOptional (fa :. xs) = bindOptional (\a -> mapOptional (\b -> a :. b) (seqOptional xs)) fa
--seqOptional (Empty :. _)   = Empty
--seqOptional (Full x :. xs) = mapOptional (\y -> x :. y) (seqOptional xs)
...
reverse ::
  List a
  -> List a
reverse = foldLeft (\x -> (:. x)) Nil
--reverse Nil       = Nil
--reverse (x :. xs) = reverse xs ++ x :. Nil  -- This implementation hangs with large lists
