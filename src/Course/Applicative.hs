...
--lift4 f = (<*>) . (lift3 f) -- is this possible?
...
--sequence (fa :. xs) = let s = foldRight (:. Nil) Nil
--                      in s <$> sequence xs
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA n fa = sequence $ loop n fa where
  loop :: Int -> f a -> List (f a)
  loop n' fa' =
    if n' > 1
    then fa' :. Nil ++ loop (n' - 1) fa'
    else fa' :. Nil
...
--filtering f (x :. xs) = ((++ acc) <$> (filtering f xs)) <*> helper where
--  helper = pure (\b -> if b then x :. Nil else Nil) <*> f x
